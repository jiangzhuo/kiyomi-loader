...
module.exports.load = function (mpath, context) {
    ...
    if (!fs.exists(mpath)) {
        throw new Error('noFile ' + mpath);
    }

    if (!fs.stat(mpath).isDirectory()) {
...
var loadPath = function (lpath, context) {
    var files = fs.readdir(lpath);
    if (files.length === 2) {
        console.warn('path is empty, path:' + lpath);
        return;
    }
    ...
    for (var i = 0, l = files.length; i < l; i++) {
        var file = files[i];
        var filePath = lpath + file.name;
        if (file.name == '.' || file.name == '..' || !file.isFile() || path.extname(filePath) != '.js') {
            // only load js file type
            continue;
        }

        m = loadFile(filePath, context);

        if (!m) {
            continue;
        }

        var name = m.name || path.basename(file.name, '.js');
        res[name] = m;
    }
